(**Representation of dynamic state data
   ...
   This module contains the state of the game playthrough, including the
   current dungeon, the list of visited dungeons, and the current fight*)
...
(**The abstract type of values representing the action taken in the
   start menu. NewGame will start a new playthrough, while Continue will
   continue an existing playthrough.*)
...
(**[init_state_from_save str] creates a new state (including the list of
   visited dungeons, player location, etc.) based on the JSON
   represented by str*)
